...
function aplicarFiltrosEBusca() {
    ...
    if (filtroCategoriaAtual !== 'todos') {
        if (['linguagem', 'framework'].includes(filtroCategoriaAtual)) {
            dadosParaRenderizar = dadosParaRenderizar.filter(dado => dado.tipo === filtroCategoriaAtual);
        } else {
            dadosParaRenderizar = dadosParaRenderizar.filter(dado => dado.categoria === filtroCategoriaAtual);
        }
    }

    // 2. Aplica a busca por texto sobre o resultado do filtro
    if (termoBuscaAtual) {
        dadosParaRenderizar = dadosParaRenderizar.filter(dado => dado.nome.toLowerCase().includes(termoBuscaAtual));
    }

    // 3. Renderiza o resultado final
    renderizarCards(dadosParaRenderizar);
}

function renderizarCards (dados){
    cardContainer.innerHTML = "";

    // Se não houver dados para renderizar, mostra uma mensagem e encerra a função
    if (dados.length === 0) {
